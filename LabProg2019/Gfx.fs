...
type CharInfo with
    ...
    static member player (fg : Color, ?bg : Color) = CharInfo.create (Config.filled_player_char, fg, ?bg = bg) 
...
type image (w, h, pixels : pixel[]) =
    ...
    static member duplicate(maze:image )=
        let img = new image ((maze.width+maze.width),maze.height)
        for row in 0..maze.height-1 do 
            for col in 0..maze.width-1 do 
                if(col= 0 )then  img.plot(col,row,maze.get(col,row))
                                 img.plot(col+1,row,maze.get(col,row))   
                else img.plot((col*2),row,maze.get(col,row))
                     img.plot((col*2+1),row,maze.get(col,row))
        img
